...
export class Tab1Page {
  ...
  ngOnInit() {
			...
			for(let b=0; b < this.characters.length; b++) {
				...
				for(let c=0; c < this.characters.length; c++) {
					...
					for(let d=0; d < this.characters.length; d++) {
						teststring = this.characters[a] + this.characters[b] + this.characters[c] + this.characters[d];
						this.tries++;
						console.log("Tries: ", this.tries);
						console.log("teststring: ", teststring);
						let match = this.matchPassword(password, teststring);
						if(match) return;
					}
				}
			}
	...
	matchPassword(password, teststring) {
		//this.testString = this.characters[a] + this.characters[b] + this.characters[c] + this.characters[d];
